/**
 * 🎵 Album Create With Token Page
 * 
 * Pagina per clienti che arrivano con un token valido da email per creare poster di album
 */
...
export function AlbumCreateWithToken() {
        ...
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8">
          {tokenData?.status === 'used' ? (
            <>
              ...
              {tokenData.downloadedAt && (
                <p className="text-sm text-gray-500 text-center mb-4">
                  Downloaded on: {new Date(tokenData.downloadedAt).toLocaleString('en-US')}
                </p>
              )}
            ...
            <>
              ...
              {tokenData.expiresAt && (
                <p className="text-sm text-gray-500 text-center mb-4">
                  Expired on: {new Date(tokenData.expiresAt).toLocaleString('en-US')}
                </p>
              )}
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-blue-800">
      ...
      <div className={`${downloadCompleted ? 'bg-orange-500' : 'bg-green-500'} text-white py-3 px-4 shadow-md`}>
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-3">
            {downloadCompleted ? (
              <>
                ...
                <span className="font-medium">Poster Already Downloaded - Link No Longer Valid</span>
              </>
            ) : (
              <>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {downloadCompleted && tokenData.downloadedAt && (
            <span className="text-sm opacity-90">
              Downloaded: {new Date(tokenData.downloadedAt).toLocaleDateString('en-US', {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric',
                hour: '2-digit',
